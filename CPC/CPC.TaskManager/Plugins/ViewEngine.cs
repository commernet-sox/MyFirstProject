...
namespace CPC.TaskManager.Plugins
{
    public class ViewEngine
    {
        ...
        public string Render(string templatePath, object model) => GetRenderDelegate(templatePath)(model);

        public string Encode(object value) => _services.Handlebars.Configuration.TextEncoder.Encode(string.Format(CultureInfo.InvariantCulture, "{0}", value));

        public string ErrorPage(Exception ex) => Render("Error.hbs", new
        {
