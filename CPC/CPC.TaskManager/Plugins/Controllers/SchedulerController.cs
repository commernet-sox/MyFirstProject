...
namespace CPC.TaskManager.Plugins.Controllers
{
    public class SchedulerController : PageControllerBase
    {
        ...
        {
            var histStore = Scheduler.Context.GetHistoryStore();
            ...
            IEnumerable<History> execHistory = null;
            ...
            if (histStore != null)
            {
                execHistory = histStore?.GetLast(10);
                executedJobs = histStore.GetTotalExecuted();
                failedJobs = histStore?.GetTotalFailed();
            }
