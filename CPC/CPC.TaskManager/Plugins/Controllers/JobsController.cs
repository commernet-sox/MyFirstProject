    ...
    public class JobsController : PageControllerBase
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            foreach (var key in keys)
            {
                ...
                knownTypes.Add(detail.JobType.GetTypeName());
                list.Add(item);
            }
        ...
        public async Task<IActionResult> Edit(string name, string group, bool clone = false)
        {
            ...
            jobModel.Type = job.JobType.GetTypeName();
        ...
        public async Task<IActionResult> Save([FromForm] JobViewModel model, bool trigger)
        {
            ...
            if (result.Success)
            {
                if (jobModel.IsNew)
                {
                    await TaskPool.AddAsync(new JobContext { Job = new Structure(jobModel.JobName, jobModel.Group, jobModel.Description) { DataMap = jobDataMap.ToDictionary(t => t.Name, t => t.Value) }, Recovery = jobModel.Recovery, JobType = Type.GetType(jobModel.Type, true) });
                }
                else
                {
                    var newJob = new JobContext
                    {
                        Job = new Structure(jobModel.JobName, jobModel.Group, jobModel.Description) { DataMap = jobDataMap.ToDictionary(t => t.Name, t => t.Value) },
                        Recovery = jobModel.Recovery,
                        JobType = Type.GetType(jobModel.Type, true),
                        JobPath = string.Empty
                    };
                    var oldKey = new JobKey(jobModel.OldJobName, jobModel.OldGroup);
                    var task = new TaskProvider(oldKey);
                    await task.UpdateJobAsync(newJob);
                }
        ...
        public async Task<IActionResult> Delete([FromBody] KeyModel model)
        {
            ...
            var task = new TaskProvider(model.ToJobKey());
            await task.DeleteJobAsync();
        ...
        public async Task<IActionResult> AdditionalData()
        {
            ...
            var history = Scheduler.Context.GetHistoryStore().GetLastOfEveryJob(10);
        ...
        [HttpGet]
        public Task<IActionResult> Duplicate(string name, string group) => Edit(name, group, clone: true);
