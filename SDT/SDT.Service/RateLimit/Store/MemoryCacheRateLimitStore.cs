...
namespace SDT.Service
{
    public class MemoryCacheRateLimitStore<T> : IRateLimitStore<T>
    {
        ...
        public MemoryCacheRateLimitStore(IMemoryCache cache) => _cache = cache;

        public Task<bool> ExistsAsync(string id, CancellationToken cancellationToken = default) => Task.FromResult(_cache.TryGetValue(id, out _));
