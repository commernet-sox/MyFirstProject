...
namespace SDT.Service
{
    public class IpRateLimitProcessor : RateLimitProcessor, IRateLimitProcessor
    {
        ...
        private readonly IRateLimitStore<List<IpRateLimitPolicy>> _policyStore;
        ...
        public async Task<IEnumerable<RateLimitRule>> GetMatchingRulesAsync(ClientRequestIdentity identity, CancellationToken cancellationToken = default)
        {
            ...
            if (!policies.IsNull())
            {
                ...
                var matchPolicies = policies.Where(r => IPUtility.ContainsIp(r.Ip, identity.ClientIp));
        ...
        {
            if (_options.IpWhitelist != null && IPUtility.ContainsIp(_options.IpWhitelist, requestIdentity.ClientIp))
            {
                return true;
            }
